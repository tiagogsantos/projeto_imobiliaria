...
class FilterController extends Controller
{
    public function search(Request $request)
    {
        ...
        // tem que ser do mesmo valor e do mesmo tipo
        if ($request->search === 'buy') {
            // se selecionar a compra vou marcar uma sessao de compra como verdadeiro
            session()->put('sale', true);
            // se caso a validação acima for verdadeira eu preciso desmarcar o rent conforme codigo abaixo
            session()->remove('rent');
            $properties = $this->createQuery('category');
        }

        if ($request->search === 'rent') {
            // se selecionar a locação vou marcar uma sessao de locação como verdadeiro
            session()->put('rent', true);
            // se caso a validação acima for verdadeira eu preciso desmarcar o buy conforme codigo abaixo
            session()->remove('sale');
            $properties = $this->createQuery('category');
        }

        // se eu conseguir fazer uma contagem dos registros
        if($properties->count()) {
            foreach($properties as $categoryProperty) {
                $category[] = $categoryProperty->category;
            }

            // estou informando todas as posições
            $collect = collect($category);
            return response()->json($this->setResponse('success', $collect->unique()->toArray()));
        }

        return response()->json($this->setResponse('fail', [], 'Ooops, não foi retornado nenhum dado
        para essa pesquisa!'));
    }
    ...
    public function category(Request $request)
    {
        ...
        return response()->json($this->setResponse('fail', [], 'Ooops, não foi retornado nenhum dado para
        essa pesquisa!'));
    }
    ...
    public function bedrooms(Request $request)
    {
        ...
        if($suitesProperties->count()){
            foreach($suitesProperties as $property){
                // se a variavel for igual a 0 ou diferente de 1
    ...
    public function garage(Request $request)
    {
        ...
        // verificando se o preço será de venda ou locação
